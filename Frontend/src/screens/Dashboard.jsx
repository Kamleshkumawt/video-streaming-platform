...
const Dashboard = () => {
  ...
  const loadVideos = useCallback(async (currentPage = 1, retries = 3) => {
    try {
      ...
      if (retries > 0) {
        console.log(`Retrying... attempts left: ${retries}`);
        setTimeout(() => loadVideos(currentPage, retries - 1), 2000);
      } else {
  ...
  // Handle video play on hover (uncomment and adjust if needed)
  // const handleMouseEnter = (videoElement) => {
  //   if (videoElement) {
  //     videoElement.play().catch((err) => console.warn('Autoplay prevented:', err));
  //   }
  ...
  const renderVideos = () => {
      ...
      return (
        <div className="col-span-full text-center py-12" role="status" aria-live="polite">
          <p className="text-gray-500 text-lg">
            No videos available. <Link to="/upload-video" className="text-blue-600 hover:underline">Upload one now</Link>.
          </p>
        </div>
      );
    }
    ...
    return videos.map((video) => (
      <div
        className="group relative bg-white rounded-xl shadow-lg hover:shadow-2xl transition-all duration-300 overflow-hidden border border-gray-200"
        key={video._id || video.filename} // Use _id for uniqueness if available
      >
        <Link to={`/video/${video.filename}`} className="block" aria-label={`View video: ${video.originalName || video.filename}`}>
          <div className="relative aspect-video overflow-hidden">
            <video
              className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-300"
              muted
              ...
              // onMouseEnter={(e) => handleMouseEnter(e.target)}
              // onMouseLeave={(e) => handleMouseLeave(e.target)}
              // poster="/path/to/default-poster.jpg" // Add if you have a poster image
            >
              <source
                src={`${config.API_BASE_URL}/videos/stream/${video.filename}`}
                type={video.mimeType || "video/mp4"} // Use dynamic MIME type from backend
              />
              Your browser does not support the video tag.
            </video>
            ...
            <div className="absolute inset-0 flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity duration-300">
              <div className="bg-white/80 rounded-full p-3">
                <svg className="w-8 h-8 text-gray-800" fill="currentColor" viewBox="0 0 20 20" aria-hidden="true">
                  <path d="M8 5v10l8-5-8-5z" />
                </svg>
              </div>
            </div>
          ...
          <div className="p-4">
            <h2 className="text-lg font-semibold text-gray-800 truncate group-hover:text-blue-600 transition-colors">
              {video.originalName || video.filename} {/* Prefer originalName */}
            </h2>
            {/* Display size or other metadata */}
            <p className="text-sm text-gray-500 mt-1">Size: {(video.size / (1024 * 1024)).toFixed(2)} MB</p>
            {/* Optional: Add duration if available in backend response */}
            {video.duration && (
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      <div className="container mx-auto px-4 py-8">
        ...
        {loading ? (
          <div className="flex justify-center items-center min-h-[75vh]" role="status" aria-live="polite">
            <TailSpin height="80" width="80" color="#4fa94d" ariaLabel="loading" />
          </div>
        ) : error ? (
          <div className="text-center py-12">
            <div className="bg-red-100 border border-red-400 text-red-700 px-4 py-3 rounded-lg max-w-md mx-auto" role="alert">
              <p className="font-medium">Error</p>
              <p>{error}</p>
              <button
                onClick={() => {
